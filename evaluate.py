...
def parse_eval_args():
    ...
    parser.add_argument("--original_embedding", default=True, action="store_true")
...
def main(args):
    ...
    stage_1_model_ckpt = Path(args.checkpoint_dir) / Path("stage_1_best.pt")
    stage_2_model_ckpt = Path(args.checkpoint_dir) / Path("stage_2_best.pt")
    print(f"Loeading stage 1 model from {stage_1_model_ckpt}")
    ...
    print(f"Loeading stage 2 model from {stage_2_model_ckpt}")
    ...
    test_metrics = evaluate(stage_2_model, test_loader, args)
