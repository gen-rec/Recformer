...
import torch
from transformers import LongformerTokenizer, PreTrainedTokenizer, BertTokenizer, RobertaTokenizer
...
class RecformerBertTokenizer(BertTokenizer):
    @classmethod
...
class RecformerRobertaTokenizer(RobertaTokenizer):
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, config=None):
        cls.config = config
        cls.attr_map: defaultdict[str, int] = defaultdict(IntFactory())  # Mapping attribute name to id
...
if __name__ == "__main__":
    ...
    tokenizer = RecformerBertTokenizer.from_pretrained("allenai/longformer-base-4096", config=config)
